...
const uk: SiteMessages = {
  nav: {
    ...
    continueWithEmailSub: "Без розширення гаманця",
    orUseWallet: "або гаманець",
    language: "Мова",
    emailLoginNeedsAppId:
      "Helius WaaS ще не готовий. Локально: HELIUS_API_KEY у .env.local + план Developer. На проді — та сама змінна в Vercel.",
    emailLoginNeedsAppIdLocal:
      "Helius WaaS ще не готовий. Потрібна підписка Developer ($49) і HELIUS_API_KEY у .env.local / Vercel, потім Redeploy.",
    ...
    googleLoginNotEnabled:
      "Вхід через Google ще не доступний у Helius. Використай email (код на пошту) або Phantom / Solflare / Jupiter.",
  ...
  deposit: {
    ...
    buyCardNeedLogin: "Увійди через email або гаманець, щоб купити карткою.",
...
const en: SiteMessages = {
  nav: {
    ...
    continueWithEmailSub: "No wallet extension needed",
    orUseWallet: "or a wallet",
    language: "Language",
    emailLoginNeedsAppId:
      "Helius WaaS is not ready yet. Locally: HELIUS_API_KEY in .env.local + Developer plan. Production: same var on Vercel.",
    emailLoginNeedsAppIdLocal:
      "Helius WaaS is not ready. Needs a Developer plan ($49) and HELIUS_API_KEY in .env.local / Vercel, then Redeploy.",
    ...
    googleLoginNotEnabled:
      "Google sign-in is not available on Helius yet. Use email (one-time code) or Phantom / Solflare / Jupiter.",
  ...
  deposit: {
    ...
    buyCardNeedLogin: "Sign in with email or a wallet to pay with a card.",
