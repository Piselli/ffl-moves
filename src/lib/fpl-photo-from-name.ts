...
type Hit = { code: number; teamId: number };
...
function add(index: Map<string, Hit[]>, key: string, hit: Hit) {
  if (!key) return;
...
function pick(hits: Hit[] | undefined, teamId?: number | null): number | null {
  if (!hits?.length) return null;
  if (teamId != null && teamId > 0) {
    const byTeam = hits.find((h) => h.teamId === teamId);
    if (byTeam) return byTeam.code;
  }
  return hits[0]!.code;
}
